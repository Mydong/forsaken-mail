...
首先是MX记录：``subdomain.domain.com MX 10 mxsubdomain.domain.com``。这意味着像``*@subdomain.domain.com``这样的地址的邮件服务器将是``mxsubdomain.domain.com``.*然后是A记录：``mxsubdomain.domain.com是.your.mailin.server``的.ip.address。这会告诉您可以在哪个ip地址找到邮件服务器。
